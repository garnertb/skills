...
### Cooldowns Don't Apply Here

When a repo has frequent CVEs, check whether a [cooldown](#cooldown-periods)
would help calm noisy version-update PRs — but make sure the user knows a
`cooldown` only delays **version updates** for newly released versions. Security
updates always go out immediately, regardless of any `cooldown` configured, so
it's not a way to "slow down" vulnerability fixes.
